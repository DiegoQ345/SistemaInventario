...
Page {
            ...
            Flickable {
                ...
                RowLayout {
                    ...
                    ColumnLayout {
                        ...
                        // Botón toggle para mostrar/ocultar el escáner simulado
                        Button {
                            id: toggleScannerButton
                            Layout.fillWidth: true
                            text: scannerRow.visible
                                  ? qsTr("\uE8BB  Ocultar Escáner Simulado")
                                  : qsTr("\uE8BB  Mostrar Escáner Simulado")
                            font.family: "Segoe MDL2 Assets"
                            flat: true
                            opacity: 0.7

                            onClicked: scannerRow.visible = !scannerRow.visible
                        }
                        ...
                        RowLayout {
                            ...
                            TextField {
                                id: testBarcodeField
                                Layout.fillWidth: true
                                placeholderText: qsTr("Código de barras (prueba)...")
                                selectByMouse: true
                                Keys.onReturnPressed: testScanButton.simulateScan()
                                Keys.onEnterPressed: testScanButton.simulateScan()
                            }

                            Button {
                                id: testScanButton
                                text: qsTr("Simular Escaneo")

                                function simulateScan() {
                                    var barcode = testBarcodeField.text.trim()
                                    if (barcode === "") return
                                    ...
                                    testBarcodeField.clear()
                                }

                                onClicked: simulateScan()
                            }
                        }
                    }
    ...
    QuantityDialog {
        ...
        onAccepted: {
            console.log("✅ Cantidad confirmada:", currentQuantity)
            if (scannedBarcode !== "") {
                viewModel.searchAndAddProduct(scannedBarcode, parseInt(currentQuantity))
                root.lastScannedBarcode = scannedBarcode
            }
        }
    }
